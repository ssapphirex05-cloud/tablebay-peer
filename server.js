...
function saveAccounts() {
  try {
    ...
    fs.writeFileSync(DATA_FILE, JSON.stringify({ accounts: list }, null, 0));
...
function ensureAcc(uid, extra) {
  ...
  if (extra) {
    ...
    if (extra.avatar != null) a.avatar = String(extra.avatar).slice(0, 400000);
  }
  return a;
}
...
app.post('/friends/accept', (req, res) => {
  ...
  me.friends.add(other.uid); other.friends.add(me.uid);
  me.incoming = (me.incoming || []).filter((x) => x.uid !== other.uid);
  saveAccounts();
  res.json({ ok: true });
});
function issueTgLink(src) {
  const uid = String((src && src.uid) || ('tg' + crypto.randomBytes(5).toString('hex')));
  const name = String((src && src.name) || '').slice(0, 24);
  const mode = String((src && src.mode) || 'login') === 'register' ? 'register' : 'login';
  const avatar = String((src && src.avatar) || '').slice(0, 400000);
  if (!TG_BOT_NAME) return { ok: false, needBot: true, hint: 'Задай TG_BOT_NAME и TG_BOT_TOKEN на Render' };
  const token = crypto.randomBytes(6).toString('hex');
  bindTokens.set(token, { uid, name, avatar, mode, ready: false, me: null, error: '', exp: Date.now() + 12 * 60 * 1000 });
  return { ok: true, token, mode, url: 'https://t.me/' + TG_BOT_NAME + '?start=bind' + token };
}
app.get('/tg/link', (req, res) => { res.json(issueTgLink(req.query || {})); });
app.post('/tg/link', (req, res) => { res.json(issueTgLink(req.body || {})); });
app.get('/tg/status', (req, res) => {
  ...
  if (rec.ready) return res.json({ ok: !rec.error, waiting: false, me: rec.me || null, existed: !!rec.existed, error: rec.error || '', hint: rec.hint || '' });
  res.json({ ok: true, waiting: true });
});
async function tgSend(chatId, text) {
  if (!TG_BOT_TOKEN) return;
  try {
    await fetch('https://api.telegram.org/bot' + TG_BOT_TOKEN + '/sendMessage', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ chat_id: chatId, text }) });
  } catch (_) {}
}
app.post('/tg', express.json(), (req, res) => {
  res.json({ ok: true });
  ...
  if (text.startsWith('/start')) {
    ...
    if (payload.startsWith('bind')) {
      ...
      const rec = bindTokens.get(token);
      if (!rec || rec.exp < Date.now()) { tgSend(chatId, 'Код устарел. Нажми вход через Telegram ещё раз в игре.'); return; }
      ...
      if (rec.mode === 'register' && existed) {
        ...
        rec.existed = true; rec.me = publicAcc(acc); saveAccounts();
        rec.hint = 'Аккаунт уже есть: «' + acc.name + '». Выполнен вход.';
        tgSend(chatId, rec.hint + ' Вернись в игру.'); return;
      }
      let acc;
      if (existed) acc = accounts.get(existingUid);
      else {
        ...
        if (rec.avatar) acc.avatar = String(rec.avatar).slice(0, 400000);
      }
      acc.tgId = tgId; acc.tgName = tgName; tgIndex.set(tgId, acc.uid);
      rec.existed = existed; rec.me = publicAcc(acc); saveAccounts();
      tgSend(chatId, existed ? ('Вход: «' + acc.name + '» (' + tgName + '). Вернись в игру.') : ('Аккаунт создан: «' + acc.name + '» (' + tgName + '). Вернись в игру.'));
      return;
    }
    const known = from.id && tgIndex.get(String(from.id));
    if (known && accounts.get(known)) {
      const acc = accounts.get(known);
      tgSend(chatId, 'Ты уже в игре как «' + acc.name + '». Новый вход — кнопка в профиле, не /start.');
      return;
    }
    tgSend(chatId, 'Это бот Tablebay. Чтобы создать аккаунт, в игре: Профиль → Регистрация через Telegram.');
  }
});
...
wss.on('connection', (ws) => {
  ...
  ws.on('message', (raw) => {
    ...
    if (msg.typ === 'create') {
      ...
      if (rooms.has(code)) { send(ws, { typ: 'error', text: 'Код занят, создай ещё раз' }); return; }
      const key = uid('h');
      ...
      send(ws, { typ: 'created', you: key, lobby: lobbyPublic(room), roster: rosterOf(room) }); return;
    }
    if (msg.typ === 'join') {
      const code = String(msg.code || '').toUpperCase().trim();
      const room = rooms.get(code);
      if (!room) { send(ws, { typ: 'error', text: 'Лобби не найдено. Хост вышел или код неверный.' }); return; }
    ...
    if (msg.typ === 'presence' && ws.room && ws.playerKey) {
      ...
      broadcast(ws.room, { typ: 'presence', player: p ? publicPlayer(p) : null, roster: rosterOf(ws.room) }); return;
    }
    if (msg.typ === 'admin' && ws.room && ws.isHost) {
      const key = msg.key;
      const p = ws.room.players.find((x) => x.key === key);
      if (p && !p.host) p.admin = !!msg.admin;
      broadcast(ws.room, { typ: 'admin', key, admin: !!(p && p.admin), roster: rosterOf(ws.room) }); return;
    }
    if ((msg.typ === 'world' || msg.typ === 'worldDone' || msg.typ === 'helloOk') && ws.room && ws.isHost) {
      if (msg.state) ws.room.world = msg.state;
      else if (msg.typ === 'worldDone' && msg.world) ws.room.world = { world: msg.world, objects: (ws.room.world && ws.room.world.objects) || [] };
    ...
    if (msg.typ === 'relay' && ws.room) {
      ...
      if (ws.isHost && (inner.typ === 'world' || inner.typ === 'worldDone' || inner.typ === 'helloOk')) {
        if (inner.state) ws.room.world = inner.state; ws.room.lastTouch = Date.now();
      }
      ...
      if (msg.to) { const target = [...ws.room.clients].find((c) => c.playerKey === msg.to); if (target) send(target, inner); return; }
      broadcast(ws.room, inner, ws); return;
    }
    if (ws.room && msg.typ && !['create', 'join', 'list', 'leave', 'ping', 'presence', 'admin'].includes(msg.typ)) {
      msg.from = ws.playerKey;
      if (ws.isHost && (msg.typ === 'world' || msg.typ === 'worldDone' || msg.typ === 'helloOk') && msg.state) ws.room.world = msg.state;
...
setInterval(() => {
  ...
  for (const [code, room] of rooms) {
    if (now - room.created > EMPTY_MS && room.players.length <= 1 && now - room.lastTouch > STALE_MS) closeRoom(code, 'Стол затих');
  }
}, 25000);
