...
app.get('/tg/link', (req, res) => {
  ...
  bindTokens.set(token, {
    uid, name, mode, ready: false, me: null, error: '',
    exp: Date.now() + 12 * 60 * 1000
  });
  res.json({ ok: true, token, mode, url: 'https://t.me/' + TG_BOT_NAME + '?start=bind' + token });
});
  ...
  if (text.startsWith('/start')) {
    ...
    if (payload.startsWith('bind')) {
      ...
      if (rec.mode === 'register' && existed) {
        ...
        tgSend(chatId, rec.hint + ' Вернись в игру.');
        return;
      }
      let acc;
      if (existed) {
      ...
      rec.me = publicAcc(acc);
      saveAccounts();
      tgSend(chatId, existed
        ? ('Вход: «' + acc.name + '» (' + tgName + '). Вернись в игру.')
        : ('Аккаунт создан: «' + acc.name + '» (' + tgName + '). Вернись в игру.'));
      return;
    }
